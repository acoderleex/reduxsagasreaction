...
export default function topList(state = defaultState, action) {
    switch (action.type) {
        case INVALIDATE:
            {
...
function getMappedTypeList(typeList) {
    ...
    return typeList.map((item, index) => {
        return {typeId: item.classificationType_id, typeName: item.classificationType_name};
    });
}
